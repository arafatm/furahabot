...
#
#   Modified by ArafatM to also return value of memories when asking for "memories"
#
# Commands:
...
#
#   hubot what do you remember - Returns keys to everything hubot remembers.
#
#   hubot memories - Returns "key - value" to everything hubot remembers.
#
...
module.exports = (robot) ->
  ...
  robot.respond /memories/i, (msg) ->
    msg.finish()
    response = []
    if memories()?
      for key,value of memories()
        response.push "- #{key} - #{value}"
      msg.send "I remember:\n#{response.join('\n')}"
    else
      msg.send "I have no memories :("
